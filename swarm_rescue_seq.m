function swarm_rescue_seq()
% Swarm Rescue (sequential multi-target, single file)
% - Slow robots
% - Random targets each run
% - Sequential engagement: target 1 -> fuse -> done -> target 2 -> ...
% - Bounding boxes visible + simple separation to prevent collisions
...
dt = 0.1; T = 240;         % step, horizon (in seconds)
...
%% -------- Visualization --------
figure('Name','Swarm Rescue — Sequential Targets (slow + bounding boxes)');
...
for k = 1:round(T/dt)
    ...
    %% 1) Compute goals (only for current target if engaging)
    goals = nan(2,N);
    if engaging && ~any(isnan(xhat(:,current)))
        for i=1:N
            goals(:,i) = xhat(:,current);
        end
    end

    %% 2) Desired velocity (search or converge)
    V_des = zeros(2,N);
    for i = 1:N
    ...
    if engaging && reportCount(current) >= k_min && trace(P(:,:,current)) < tau_cov
        ...
        engaging = false;
        centroid_guess = [NaN; NaN];
        % If there is a next target, keep going; otherwise loop will end at top.
    end
